...
class Settings(BaseSettings):
    ...
    pii_ner_mode: str = Field(default="efficiency", description="PERSON/LOCATION/ORGANIZATION detection strategy: 'efficiency' (bundled spaCy model, fast) or 'accuracy' (HuggingFace radlab/pii-pl-v1.0 transformer, slower but more accurate; requires the 'accuracy' extra)")
    pl_ner_model_path: str = Field(default="models/pii_ner_model", description="Filesystem path to the bundled spaCy NER pipeline used in 'efficiency' mode")
    pii_ner_hf_model: str = Field(default="radlab/pii-pl-v1.0", description="HuggingFace model id (or local path) for the token-classification pipeline used in 'accuracy' mode")
